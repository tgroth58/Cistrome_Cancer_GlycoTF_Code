...
#Load the TF-glycosylation pathway data.  Loaded as "ctype_res"
...
load(file.path(f_path,ctype_enrichData))
...
res<-sapply(unique(ctype_res$glycoPath),function(pth){
	#Filter total_tf_glyco_dta_new for this combination of 
	# path and cancer type and pvalue<=0.05
	dta<-ctype_res %>% filter(glycoPath==pth,pEnrich<=0.05)
